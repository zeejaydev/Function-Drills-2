...
// CODE HERE
const totalsAfterTax = orders.map((order)=>order.price * (1 + order.tax) ).reduce((acc,current)=>acc+current)
console.log(totalsAfterTax)
...
// CODE HERE
const bobsTotal = purchases.filter((purchas)=>purchas.owner === 'Bob').map((e)=>e.price).reduce((acc,current)=>acc+current)
